...
public class CommandRegister {
    private static final SuggestionProvider<FabricClientCommandSource> PLAYERS = (context, suggestionsBuilder) -> suggestPlayers(suggestionsBuilder);
    ...
    private static CompletableFuture<Suggestions> suggestPlayers(SuggestionsBuilder suggestionsBuilder) {
        ...
        for (PlayerInfo playerListEntry : minecraft.getConnection().getOnlinePlayers())
            if (SharedSuggestionProvider.matchesSubStr(suggestionsBuilder.getRemaining().toLowerCase(Locale.ROOT), playerListEntry.getProfile().getName().toLowerCase(Locale.ROOT)) && playerListEntry.getProfile().getName().length() > 2)
    ...
    public static void registerCommands() {
        ClientCommandRegistrationCallback.EVENT.register((commandDispatcher, registryAccess) -> commandDispatcher.register(
