...
public class GameMode {
    ...
    public void setDirectTier(String fullTier, String fullPeakTier) {
        if (fullTier == null || fullTier.isEmpty() || fullTier.equalsIgnoreCase("NONE") || fullTier.equalsIgnoreCase("0")) {
            status = Status.NOT_EXISTING;
            return;
        }

        displayedTierUnformatted = fullTier.toUpperCase(Locale.ROOT);
        boolean retired = displayedTierUnformatted.startsWith("R");
        String cleanTier = retired ? displayedTierUnformatted.substring(1) : displayedTierUnformatted;
        tier = cleanTier.replaceAll("[^0-9]", "");
        ...
        if (cleanTier.startsWith("HT") || cleanTier.startsWith("H"))
            tierTooltipString += "High ";
        else if (cleanTier.startsWith("LT") || cleanTier.startsWith("L"))
            tierTooltipString += "Low ";
        tierTooltipString += "Tier " + tier + "\n\nPoints: " + getTierPoints(false);

        tierTooltip = Component.literal(tierTooltipString).setStyle(Style.EMPTY.withColor(getTierColor(displayedTierUnformatted)));

        if (fullPeakTier != null && !fullPeakTier.isEmpty() && !fullPeakTier.equalsIgnoreCase("NONE") && !fullPeakTier.equalsIgnoreCase(fullTier)) {
            displayedPeakTierUnformatted = fullPeakTier.toUpperCase(Locale.ROOT);
            String cleanPeak = displayedPeakTierUnformatted.startsWith("R") ? displayedPeakTierUnformatted.substring(1) : displayedPeakTierUnformatted;
            peakTier = cleanPeak.replaceAll("[^0-9]", "");

            displayedPeakTier = Component.literal("(" + displayedPeakTierUnformatted + ")").setStyle(Style.EMPTY.withColor(getTierColor(displayedPeakTierUnformatted)));

            String peakTooltipString = "Peak: ";
            if (displayedPeakTierUnformatted.startsWith("R"))
                peakTooltipString += "Retired ";
            if (cleanPeak.startsWith("HT") || cleanPeak.startsWith("H"))
                peakTooltipString += "High ";
            else if (cleanPeak.startsWith("LT") || cleanPeak.startsWith("L"))
                peakTooltipString += "Low ";
            peakTooltipString += "Tier " + peakTier + "\n\nPoints: " + getTierPoints(true);

            peakTierTooltip = Component.literal(peakTooltipString).setStyle(Style.EMPTY.withColor(getTierColor(displayedPeakTierUnformatted)));
            hasPeak = true;
        }
