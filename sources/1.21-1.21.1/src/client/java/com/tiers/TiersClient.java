...
public class TiersClient implements ClientModInitializer {
    ...
    public static DisplayStatus positionCZSKTiers = DisplayStatus.OFF;
    public static Mode activeCZSKTiersMode = Mode.CZSK_SWORD;

    public static DisplayStatus positionCZSKB0t = DisplayStatus.OFF;
    public static Mode activeCZSKB0tMode = Mode.CZSK_B0T_SWORD;

    public static DisplayStatus positionCZSKSubtiers = DisplayStatus.OFF;
    public static Mode activeCZSKSubtiersMode = Mode.CZSK_SUB_SPEED;
    ...
    public void onInitializeClient() {
        ...
        modContainer.ifPresent(tiers -> {
            ResourceManagerHelper.registerBuiltinResourcePack(ResourceLocation.fromNamespaceAndPath("resourcepacks", "tiers-resources"), tiers, Component.literal("Resources for Tiers"), ResourcePackActivationType.ALWAYS_ENABLED);
            userAgent += " v" + tiers.getMetadata().getVersion().getFriendlyString();
        });

        String category = "key.category.tiers";
        ...
        Minecraft.getInstance().execute(() -> {
            playerProfiles.forEach(playerProfile -> {
                if (playerProfile.status == Status.READY) {
                    if (playerProfile.profileMCTiers != null && playerProfile.profileMCTiers.status == Status.READY)
                    ...
                    if (playerProfile.profileSubtiers != null && playerProfile.profileSubtiers.status == Status.READY)
    ...
    public static Component cycleRightMode() {
        ...
        if (positionCZSKTiers.toString().equalsIgnoreCase("RIGHT"))
            return Component.literal("Right (CZSKTiers) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKTiersMode());

        if (positionCZSKB0t.toString().equalsIgnoreCase("RIGHT"))
            return Component.literal("Right (CZSK-B0t) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKB0tMode());

        if (positionCZSKSubtiers.toString().equalsIgnoreCase("RIGHT"))
            return Component.literal("Right (CZSK-Subtiers) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKSubtiersMode());
    ...
    public static Component cycleLeftMode() {
        ...
        if (positionCZSKTiers.toString().equalsIgnoreCase("LEFT"))
            return Component.literal("Left (CZSKTiers) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKTiersMode());

        if (positionCZSKB0t.toString().equalsIgnoreCase("LEFT"))
            return Component.literal("Left (CZSK-B0t) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKB0tMode());

        if (positionCZSKSubtiers.toString().equalsIgnoreCase("LEFT"))
            return Component.literal("Left (CZSK-Subtiers) is now displaying ").setStyle(Style.EMPTY.withColor(CommonColors.WHITE)).append(cycleCZSKSubtiersMode());
    ...
    public static Component getRightIcon() {
        ...
        if (positionCZSKTiers.toString().equalsIgnoreCase("RIGHT"))
            return activeCZSKTiersMode.getIcon();

        if (positionCZSKB0t.toString().equalsIgnoreCase("RIGHT"))
            return activeCZSKB0tMode.getIcon();

        if (positionCZSKSubtiers.toString().equalsIgnoreCase("RIGHT"))
            return activeCZSKSubtiersMode.getIcon();
    ...
    public static void tiersCommand(String playerName) {
        if (playerName.equalsIgnoreCase("-toggle"))
            ...
            sendMessageToPlayer(Component.literal("- Report a bug: ").append(Component.literal("Tiers GitHub issues").withStyle(style -> style.withUnderlined(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/PvPTiers/Tiers/issues")))), false);
            sendMessageToPlayer(Component.literal("- It's not advisable to create tickets in PvPTiers support"), false);
            sendMessageToPlayer(Component.literal("- ").append(Component.literal("Changelogs").withStyle(style -> style.withUnderlined(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/PvPTiers/Tiers/wiki/Version-changelogs")))), false);
            sendMessageToPlayer(Component.literal("- ").append(Component.literal("Modrinth page").withStyle(style -> style.withUnderlined(true).withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://modrinth.com/mod/tiers")))), false);
            ...
            sendMessageToPlayer(Icons.colorText("CZSKTiers requests failed: " + SuperProfile.failedCZSKTiersRequests + "/" + SuperProfile.CZSKTiersRequests + " (failed / requested)", CommonColors.YELLOW), false);
            sendMessageToPlayer(Icons.colorText("CZSK-B0t requests failed: " + SuperProfile.failedCZSKB0tRequests + "/" + SuperProfile.CZSKB0tRequests + " (failed / requested)", CommonColors.YELLOW), false);
            sendMessageToPlayer(Icons.colorText("CZSK-Subtiers requests failed: " + SuperProfile.failedCZSKSubtiersRequests + "/" + SuperProfile.CZSKSubtiersRequests + " (failed / requested)", CommonColors.YELLOW), false);
            sendMessageToPlayer(Icons.colorText("", CommonColors.WHITE), false);
            ...
            sendMessageToPlayer(Icons.colorText("CZSKTiers status | is down? " + SuperProfile.isCZSKTiersDown + " | Failed request in last minute: " + SuperProfile.failedCZSKTiersRequestsLastMinute, CommonColors.YELLOW), false);
            sendMessageToPlayer(Icons.colorText("CZSK-B0t status | is down? " + SuperProfile.isCZSKB0tDown + " | Failed request in last minute: " + SuperProfile.failedCZSKB0tRequestsLastMinute, CommonColors.YELLOW), false);
            sendMessageToPlayer(Icons.colorText("CZSK-Subtiers status | is down? " + SuperProfile.isCZSKSubtiersDown + " | Failed request in last minute: " + SuperProfile.failedCZSKSubtiersRequestsLastMinute, CommonColors.YELLOW), false);
    ...
    public static String[] getDebugInfo() {
        ...
        debugInfo[1] += "Version type: " + SharedConstants.getCurrentVersion().getName() + "\n";
        debugInfo[1] += "Game profile name: " + Minecraft.getInstance().getGameProfile().getName() + "\n";
    ...
    public static void changeIcons(Icons.Type iconType, boolean reload) {
        Icons.identifierMCTiers = ResourceLocation.fromNamespaceAndPath("minecraft", "gamemodes/" + iconType.name().toLowerCase(Locale.ROOT));
        Icons.identifierPvPTiers = ResourceLocation.fromNamespaceAndPath("minecraft", "gamemodes/" + iconType.name().toLowerCase(Locale.ROOT));
        Icons.identifierMCTiersTags = ResourceLocation.fromNamespaceAndPath("minecraft", "gamemodes/" + iconType.name().toLowerCase(Locale.ROOT) + "-tags");
        Icons.identifierPvPTiersTags = ResourceLocation.fromNamespaceAndPath("minecraft", "gamemodes/" + iconType.name().toLowerCase(Locale.ROOT) + "-tags");
    ...
    public static void updateCaches() {
        ...
        for (Entity entity : minecraft.level.entitiesForRendering())
            if (entity instanceof Display.TextDisplay textDisplay)
                ((DataTrackerAccessor) textDisplay.getEntityData()).invokeSet(TextDisplayAccessor.getTEXT(), ((TextDisplayAccessor) textDisplay).invokeGetText(), true);
    }
    ...
    public static Component cycleCZSKTiersMode() {
        activeCZSKTiersMode = cycleEnum(activeCZSKTiersMode, Mode.getCZSKTiersValues());
        ConfigManager.saveConfig();
        return activeCZSKTiersMode.getTextLabel();
    }

    public static Component cycleCZSKB0tMode() {
        activeCZSKB0tMode = cycleEnum(activeCZSKB0tMode, Mode.getCZSKB0tValues());
        ConfigManager.saveConfig();
        return activeCZSKB0tMode.getTextLabel();
    }

    public static Component cycleCZSKSubtiersMode() {
        activeCZSKSubtiersMode = cycleEnum(activeCZSKSubtiersMode, Mode.getCZSKSubtiersValues());
        ConfigManager.saveConfig();
        return activeCZSKSubtiersMode.getTextLabel();
    }
