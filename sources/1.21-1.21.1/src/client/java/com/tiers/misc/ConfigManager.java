...
public class ConfigManager {
    ...
    private static class Config {
        ...
        TiersClient.DisplayStatus positionCZSKTiers;
        Mode activeCZSKTiersMode;

        TiersClient.DisplayStatus positionCZSKB0t;
        Mode activeCZSKB0tMode;

        TiersClient.DisplayStatus positionCZSKSubtiers;
        Mode activeCZSKSubtiersMode;
    ...
    public static void loadConfig() {
        ...
        if (Arrays.stream(TiersClient.DisplayStatus.values()).toList().contains(config.positionCZSKTiers))
            TiersClient.positionCZSKTiers = config.positionCZSKTiers;
        if (Arrays.stream(Mode.values()).toList().contains(config.activeCZSKTiersMode) && config.activeCZSKTiersMode.toString().contains("CZSKTIERS"))
            TiersClient.activeCZSKTiersMode = config.activeCZSKTiersMode;

        if (Arrays.stream(TiersClient.DisplayStatus.values()).toList().contains(config.positionCZSKB0t))
            TiersClient.positionCZSKB0t = config.positionCZSKB0t;
        if (Arrays.stream(Mode.values()).toList().contains(config.activeCZSKB0tMode) && config.activeCZSKB0tMode.toString().contains("CZSK_B0T"))
            TiersClient.activeCZSKB0tMode = config.activeCZSKB0tMode;

        if (Arrays.stream(TiersClient.DisplayStatus.values()).toList().contains(config.positionCZSKSubtiers))
            TiersClient.positionCZSKSubtiers = config.positionCZSKSubtiers;
        if (Arrays.stream(Mode.values()).toList().contains(config.activeCZSKSubtiersMode) && config.activeCZSKSubtiersMode.toString().contains("CZSK_SUBTIERS"))
            TiersClient.activeCZSKSubtiersMode = config.activeCZSKSubtiersMode;
            ...
            ClientTickEvents.END_CLIENT_TICK.register(minecraft -> {
                ...
                if (minecraft.screen instanceof net.minecraft.client.gui.screens.TitleScreen) {
                    ...
                    if (launchTickCounter >= 20) {
                        SystemToast.add(minecraft.getToasts(), SystemToast.SystemToastId.NARRATOR_TOGGLE, Component.literal("Thanks for updating Tiers"), Component.literal("Some settings may have changed"));
    ...
    private static void updateConfig(Config config) {
        ...
        config.positionCZSKTiers = TiersClient.positionCZSKTiers;
        config.activeCZSKTiersMode = TiersClient.activeCZSKTiersMode;

        config.positionCZSKB0t = TiersClient.positionCZSKB0t;
        config.activeCZSKB0tMode = TiersClient.activeCZSKB0tMode;

        config.positionCZSKSubtiers = TiersClient.positionCZSKSubtiers;
        config.activeCZSKSubtiersMode = TiersClient.activeCZSKSubtiersMode;
    ...
    public static String getCurrentConfig() {
        return "\nConfig{" +
                ...
                "\npositionCZSKTiers=" + config.positionCZSKTiers +
                "\nactiveCZSKTiersMode=" + config.activeCZSKTiersMode +
                "\npositionCZSKB0t=" + config.positionCZSKB0t +
                "\nactiveCZSKB0tMode=" + config.activeCZSKB0tMode +
                "\npositionCZSKSubtiers=" + config.positionCZSKSubtiers +
                "\nactiveCZSKSubtiersMode=" + config.activeCZSKSubtiersMode +
